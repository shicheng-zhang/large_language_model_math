...
tensor* tensor_softmax(tensor* target_tensor) {
    ...
    for (size_t i = 0; i < target_tensor->row_count; i++) {
        ...
        for (size_t j = 1; j < target_tensor->column_count; j++) {
            if (target_tensor->data[i * target_tensor->column_count + j]->data > maximum_data) {
                maximum_data = target_tensor->data[i * target_tensor->column_count + j]->data;
            }
        }
        ...
        for (size_t j = 0; j < target_tensor->column_count; j++) {
            ...
            value_free(sum); value_free(shifted);
            sum = new_sum;
        }
        ...
        value_free(sum); value_free(inverse_sum); free(exponentials);
    }
    return output_tensor;
}
...
tensor* tensor_attention(tensor* query_tensor, tensor* key_tensor, tensor* value_tensor) {
    ...
    if (!query_key_product) { tensor_free(transposed_key_tensor); return NULL; }

    tensor* scaled_tensor = tensor_scalar_multiplication(query_key_product, 1.0 / sqrt((double)key_tensor->column_count));
    if (!scaled_tensor) { tensor_free(query_key_product); tensor_free(transposed_key_tensor); return NULL; }

    tensor* attention_weights = tensor_softmax(scaled_tensor);
    if (!attention_weights) { tensor_free(scaled_tensor); tensor_free(query_key_product); tensor_free(transposed_key_tensor); return NULL; }

    tensor* output_tensor = tensor_matrix_multiplication(attention_weights, value_tensor);
    if (!output_tensor) { tensor_free(attention_weights); tensor_free(scaled_tensor); tensor_free(query_key_product); tensor_free(transposed_key_tensor); return NULL; }

    tensor_free(transposed_key_tensor); tensor_free(query_key_product); tensor_free(scaled_tensor); tensor_free(attention_weights);
    return output_tensor;
}
