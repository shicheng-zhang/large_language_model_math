...
tensor* tensor_layer_normalization(tensor* target_tensor, double epsilon) {
    ...
    for (size_t i = 0; i < target_tensor->row_count; i++) {
        ...
        for (size_t j = 0; j < target_tensor->column_count; j++) {
            ...
            value_free(variance_sum); value_free(squared);
        ...
        value_free(variance); value_free(epsilon_value);
        ...
        value_free(mean); value_free(inverse_standard_deviation);
    }
...
tensor* tensor_rotary_positional_embedding(tensor* target_tensor, int position, double base) {
    ...
    for (size_t i = 0; i < target_tensor->row_count; i++) {
        for (size_t j = 0; j < half_dim; j++) {
            ...
            value_free(output_tensor->data[idx1]); value_free(output_tensor->data[idx2]);
            ...
            value_free(cosine_value); value_free(sine_value);
        }
    }
    return output_tensor;
}
