...
adam* adam_new(size_t row_count, size_t column_count, double learning_rate) {
    ...
    optimizer->element_count = row_count * column_count;
    // V4: Raw double arrays, decoupled from autograd graph
...
void adam_free(adam* optimizer) {
    if (!optimizer) return;
    free(optimizer->first_moment);
    free(optimizer->second_moment);
    free(optimizer);
}
