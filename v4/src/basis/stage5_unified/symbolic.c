...
symbol* symbol_subtraction(symbol* first_operand, symbol* second_operand) {
    symbol* new_symbol = symbol_operation_new("-", 2);
    new_symbol->arguments[0] = first_operand; new_symbol->arguments[1] = second_operand;
    return new_symbol;
}
...
symbol* symbol_power(symbol* base, double exponent) {
    ...
    new_symbol->arguments[0] = base; new_symbol->value = exponent;
    return new_symbol;
}
...
symbol* symbol_copy(symbol* source_symbol) {
    ...
    if (source_symbol->type == symbol_type_variable) {
        output_symbol = symbol_variable(source_symbol->name);
    } else if (source_symbol->type == symbol_type_constant) {
        output_symbol = symbol_constant(source_symbol->value);
    } else {
        ...
        for (size_t i = 0; i < source_symbol->argument_count; i++) {
            output_symbol->arguments[i] = symbol_copy(source_symbol->arguments[i]);
        }
    }
...
symbol* symbol_differentiation(symbol* source_symbol, char* variable_name) {
    ...
    if (source_symbol->type == symbol_type_operation) {
        ...
        if (strcmp(source_symbol->operation, "-") == 0) return symbol_subtraction(symbol_differentiation(source_symbol->arguments[0], variable_name), symbol_differentiation(source_symbol->arguments[1], variable_name));
...
symbol* symbol_simplify(symbol* source_symbol) {
    ...
    if (strcmp(source_symbol->operation, "+") == 0) {
        ...
        if (first_argument->type == symbol_type_constant && second_argument->type == symbol_type_constant) {
            ...
            symbol_free(first_argument); symbol_free(second_argument); return symbol_constant(result);
        }
        ...
        if (first_argument->type == symbol_type_constant) {
            ...
            symbol_free(first_argument); return symbol_constant(result);
        }
