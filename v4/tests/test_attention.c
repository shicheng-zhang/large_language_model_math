...
int main() {
    printf("Testing Transformer Attention Math (V4)...\n");
    tensor* query_tensor = tensor_new(1, 2); tensor_set(query_tensor, 0, 0, 1.0); tensor_set(query_tensor, 0, 1, 0.0);
    tensor* key_tensor = tensor_new(1, 2); tensor_set(key_tensor, 0, 0, 1.0); tensor_set(key_tensor, 0, 1, 0.0);
    tensor* value_tensor = tensor_new(1, 2); tensor_set(value_tensor, 0, 0, 10.0); tensor_set(value_tensor, 0, 1, 20.0);
    ...
    tensor_free(output_tensor); tensor_free(query_tensor); tensor_free(key_tensor); tensor_free(value_tensor);
    return 0;
}
