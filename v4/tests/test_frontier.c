...
int main() {
    printf("Frontier Convergence Test: Learning x * 2 = 10 (V4 Optimizer)\n");
    tensor* weight_tensor = tensor_new(1, 1); tensor_set(weight_tensor, 0, 0, 0.5);
    tensor* input_tensor = tensor_new(1, 1); tensor_set(input_tensor, 0, 0, 5.0);
    ...
    for (int epoch = 0; epoch < 50; epoch++) {
        ...
        if (epoch % 10 == 0) printf("Epoch %d: Loss = %f, W = %f\n", epoch, loss->data, weight_tensor->data[0]->data);

        value_backward_propagation(loss);
        value_free(loss); value_free(difference); value_free(target_value);
    ...
    adam_free(optimizer); tensor_free(weight_tensor); tensor_free(input_tensor);
    return 0;
}
