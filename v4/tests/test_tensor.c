...
int main() {
    printf("Testing Tensor MatMul and Autograd (V4)...\n");
    tensor* input_tensor = tensor_new(1, 2);
    tensor_set(input_tensor, 0, 0, 1.0); tensor_set(input_tensor, 0, 1, 2.0);

    tensor* weight_tensor = tensor_new(2, 2);
    tensor_set(weight_tensor, 0, 0, 3.0); tensor_set(weight_tensor, 0, 1, 4.0);
    tensor_set(weight_tensor, 1, 0, 5.0); tensor_set(weight_tensor, 1, 1, 6.0);
    ...
    value_free(loss); tensor_free(output_tensor); tensor_free(input_tensor); tensor_free(weight_tensor);
    return 0;
}
